...
class NEURON: 
    ...
    def Update_Hidden_Or_Motor_Neuron(self, neurons, synapses):
        ...
        for synKey in synapses.keys():
            source, dest = synKey
            if dest == self.Get_Name():
                self.Allow_Presynaptic_Neuron_To_Influence_Me(synapses[synKey].Get_Weight(), neurons[source].Get_Value())
        self.Threshold()

    def Allow_Presynaptic_Neuron_To_Influence_Me(self, weight, preSynValue):
        self.Add_To_Value(weight * preSynValue)
